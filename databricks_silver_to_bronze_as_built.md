# [Pipeline Name] – Databricks Silver → Bronze As-Built Documentation
...
**Purpose:** Brief description of why this pipeline exists and what business/technical need it addresses (e.g. reprocessing, archival, correction, or re-ingestion of Silver data back into Bronze).
...
1. Read from Silver source (filters, incremental logic, watermarking)
2. Transformations applied (column drops/renames, dedup, flattening, etc.)
3. Write to Bronze target (merge keys, write mode)

## 9. Schema Mapping

| Source Column (Silver) | Target Column (Bronze) | Data Type | Transformation/Notes |
|---|---|---|---|
| | | | |
...
- **Downstream:** consumers of the Bronze output (dashboards, other jobs, ML pipelines)
